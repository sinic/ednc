...
Icons with transparency are not fully supported yet:
Emacs does not support [[http://netpbm.sourceforge.net/doc/pam.html][PAM]],
a newer Netpbm format with an alpha channel.
Until that changes, transparency information is simply ignored.

*** Why are some icons not showing up at all?
Only file URIs are supported for now: the
[[https://specifications.freedesktop.org/icon-theme-spec/icon-theme-spec-latest.html][Icon Theme Specification]]
