...
<ul><li><kbd>Return</kbd> or <kbd>mouse-1</kbd> (left click)
        invokes the notification's default action
        (<code>ednc-invoke-action</code>),</li>
    <li><kbd>C-mouse-1</kbd>
        pops to the notification in the interactive log buffer
	(<code>ednc-pop-to-notification-in-log-buffer</code>),</li>
	...
	(<code>ednc-dismiss-notification</code>).</li></ul>
#+END_html
