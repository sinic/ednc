...
execute the ~dnel-mode~ command to activate the global minor mode, and either
switch to the buffer returned by the ~dnel-generate-log-buffer~ function, or
simply wait for the first notification to be logged in the background
(which automatically creates the interactive log buffer ~*dnel-log*~).
